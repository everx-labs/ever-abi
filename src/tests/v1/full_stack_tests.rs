...
use ton_types::{BuilderData, SliceData, ed25519_generate_private_key, Ed25519PublicKey};
...
fn test_signed_call() {
    ...
    assert!(Ed25519PublicKey::from_bytes(&key.verifying_key()).unwrap().verify(hash.as_slice(), &sign.try_into().unwrap()));
