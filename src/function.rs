...
use ton_types::{error, fail, BuilderData, IBitstring, Result, SliceData, MAX_DATA_BYTES};
...
impl Function {
    ...
    pub fn get_function_signature(&self) -> String {
        ...
        if self.abi_version.major == 1 {
            input_types.extend(self.header.iter().map(|param| param.kind.type_signature()));
        }

        input_types.extend(self.inputs.iter().map(|param| param.kind.type_signature()));
        ...
        if !internal {
                ...
                if reserve_sign {
                    if self.abi_version >= ABI_VERSION_2_3 {
                        sign_builder.append_raw(
                            &[0u8; MAX_DATA_BYTES],
                            ParamType::Address.max_bit_size(),
                        )?;
            ...
            cells.insert(
                ...
                SerializedValue {
                    ...
                    max_refs: usize::from(remove_ref),
                },
            );
        }
