...
mod tokenize_tests {
    ...
    fn test_tokenize_cell() {
        ...
        builder.append_reference(
            BuilderData::with_bitstring(vec![11, 12, 13, 14, 15, 16, 17, 18, 0x80]).unwrap(),
        );
        builder.append_reference(
            BuilderData::with_bitstring(vec![21, 22, 23, 24, 25, 26, 27, 28, 0x80]).unwrap(),
        );
...
mod types_check_tests {
    ...
    fn test_type_check() {
        ...
        let mut tokens_wrong_tuple_type = tokens;
        tokens_wrong_tuple_type[9] = Token {
            name: "f".to_owned(),
            value: TokenValue::Tuple(vec![
                Token {
                    name: "a".to_owned(),
                    value: TokenValue::Int(big_int),
                },
                Token {
                    name: "b".to_owned(),
                    value: TokenValue::Uint(big_uint),
                },
            ]),
        };
