...
impl Contract {
    ...
    pub fn update_data(&self, data: SliceData, tokens: &[Token]) -> Result<SliceData> {
        ...
        for token in tokens {
            ...
            let key = self.data
                ...
                map.set_builder(
                    key.write_to_new_cell().unwrap().into_cell()?.into(),
                    &builder, 
                )?;
        }

        Ok(map.write_to_new_cell()?.into_cell()?.into())
    }
    ...
    pub fn get_pubkey(data: &SliceData) -> Result<Option<Vec<u8>>> {
        ...
        map.get(0u64.write_to_new_cell()?.into_cell()?.into())
    ...
    pub fn insert_pubkey(data: SliceData, pubkey: &[u8]) -> Result<SliceData> {
        ...
        map.set_builder(
            0u64.write_to_new_cell().unwrap().into_cell()?.into(),
            &value, 
        )?;
        Ok(map.write_to_new_cell()?.into_cell()?.into())
    }
