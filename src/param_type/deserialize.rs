...
pub fn read_type(name: &str) -> Result<ParamType> {
    ...
    if let Some(']') = name.chars().last() {
        ...
        if num.is_empty() {
            ...
            // it's a fixed array.
            let len = num.parse().map_err(|_| AbiError::InvalidName {
                name: name.to_owned(),
            })?;
    ...
    let result = match name {
        ...
        s if s.starts_with("int") => {
            let len = (s[3..]).parse().map_err(|_| AbiError::InvalidName {
                name: name.to_owned(),
            })?;
            ParamType::Int(len)
        }
        s if s.starts_with("uint") => {
            let len = (s[4..]).parse().map_err(|_| AbiError::InvalidName {
                name: name.to_owned(),
            })?;
            ParamType::Uint(len)
        }
        s if s.starts_with("varint") => {
            let len = (s[6..]).parse().map_err(|_| AbiError::InvalidName {
                name: name.to_owned(),
            })?;
            ParamType::VarInt(len)
        }
        s if s.starts_with("varuint") => {
            let len = (s[7..]).parse().map_err(|_| AbiError::InvalidName {
                name: name.to_owned(),
            })?;
        ...
        s if s.starts_with("fixedbytes") => {
            let len = (s[10..]).parse().map_err(|_| AbiError::InvalidName {
                name: name.to_owned(),
            })?;
